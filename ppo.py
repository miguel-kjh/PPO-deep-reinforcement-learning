...
def main():
    ...
    initial_observation, _ = envs.reset()
    ...
    for update in range(1, num_updates+1):
        if args.anneal_lr:
            ...
            for param_group in optimizer.param_groups:
                param_group["lr"] = args.learning_rate - (args.learning_rate * (update / num_updates))
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                action, logproba, _, value = agent.get_action_and_value(obs[step])
                values[step] = value.flatten()
                actions[step] = action
                logprobs[step] = logproba

            # TRY NOT TO MODIFY: execute the game and log data.
            next_obs, rs, ds, tr, infos = envs.step(action.cpu().numpy())
